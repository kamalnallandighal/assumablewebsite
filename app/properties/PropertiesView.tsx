...
interface Props {
  initialListings: readonly Listing[];
}

export function PropertiesView({ initialListings }: Props) {
  return (
    <Suspense fallback={<div className="h-screen" />}>
      <PropertiesViewBody initialListings={initialListings} />
    </Suspense>
  );
}

function PropertiesViewBody({ initialListings }: Props) {
  ...
  const { filters, visible: filteredVisible, chipFiltered, patch } = useFilters(
    initialListings,
    bbox
  );
  ...
  // Look up the listing for the modal from the in-memory set (no roundtrip).
  const detailListing = detailId
    ? initialListings.find((l) => l.id === detailId) ?? null
    : null;
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-col md:grid md:grid-cols-[1fr_540px] md:grid-rows-1">
        ...
        >
          <PropertiesMap
            ...
            initialBbox={bbox}
            fitListings={chipFiltered.length > 0 ? chipFiltered : initialListings}
